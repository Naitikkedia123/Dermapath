...
app.post('/upload-report', upload.single('report'), async (req, res) => {
  try {
    ...
    if (req.file.size > 2 * 1024 * 1024) {
      return res.redirect('/patient-dashboard?error=File size exceeds 2MB');
    }
    const result = await cloudinary.uploader.upload(req.file.path, {
   ...
   patient.medicalReports.push({
    ...
    console.error(err);
    res.redirect('/patient-dashboard?error=Failed to upload report');
  }
});
...
app.get('/end-treatment', isPatient, async (req, res) => {
  ...
  try {
    ...
    // ✅ Delete all Cloudinary images from Chat
    const imageChats = await Chat.find({
      $or: [
        { from: patient._id, to: doctorId },
        { from: doctorId, to: patient._id }
      ],
      type: 'image'
    });

    for (const chat of imageChats) {
      if (chat.public_id) {
        await cloudinary.uploader.destroy(chat.public_id, { resource_type: 'image' });
      }
    }


    // ✅ Delete patient-doctor records
    patient.doctors = [];
    await patient.save();
...
app.post('/delete-report', async (req, res) => {
  try {
    ...
    const reportToDelete = patient.medicalReports.find(r => r.url === reportUrl);

    if (reportToDelete && reportToDelete.public_id) {
      await cloudinary.uploader.destroy(reportToDelete.public_id, { resource_type: 'raw' });
    }

    patient.medicalReports = patient.medicalReports.filter(report => report.url !== reportUrl);
    await patient.save();

    res.redirect('/patient-dashboard');
  } catch (err) {
    console.error(err);
    res.redirect('/patient-dashboard?error=Failed to delete report');
  }
});
